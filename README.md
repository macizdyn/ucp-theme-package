...
Add the service provider in `app/config/app.php`, `Providers` array:
    ...
    macizdyn\ucpTheme\themeServiceProvider::class,

also edit the `Facades` array and add:

    'Theme' => macizdyn\ucpTheme\Facades\Theme::class,

Almost Done. You can optionally publish a configuration file to your application with

    php artisan vendor:publish --provider=macizdyn\ucpTheme\themeServiceProvider
...
```
...or you can use a middleware to apply a theme to a `Route::group()` etc. For more advanced example check demo application: [Set Theme in Session](https://github.com/macizdyn/ucp-theme-demo)
